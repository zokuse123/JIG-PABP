...
export default function KeuanganPage({ bookings }) {
  const [selId, setSelId] = useState(bookings[0]?.id || null);
  const b = bookings.find((x) => x.id === selId);

  const profit      = b ? hitungProfit(b) : 0;
  const sisaTagihan = b ? b.hargaDeal - b.dp : 0;
        ...
        {b ? (
          <Card>
            ...
            <div style={{ paddingTop: 16, marginTop: 4 }}>
              ...
              >
                {profit >= 0
                  ? `✅ Margin ${((profit / b.hargaDeal) * 100).toFixed(1)}% dari harga deal`
